...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex justify-between items-center bg-white dark:bg-dark-800 p-6 rounded-2xl shadow-glass border border-slate-100 dark:border-dark-700">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white">Dashboard Institucional</h1>
          <p className="text-slate-500 dark:text-slate-400 text-sm mt-1">Resumen financiero y estado de asociados al {CURRENT_DATE_MOCK.toLocaleDateString()}</p>
        </div>
        <div className="flex shadow-sm rounded-xl px-4 py-2 bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-400 font-semibold items-center border border-primary-100 dark:border-primary-800/30">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="premium-card relative overflow-hidden group">
          ...
          {isLoading ? <div className="skeleton h-10 w-24"></div> : (
            <div>
        ...
        <div className="premium-card relative overflow-hidden group">
          ...
          {isLoading ? <div className="skeleton h-10 w-32"></div> : (
            <div>
        ...
        <div className="premium-card relative overflow-hidden group">
          ...
          {isLoading ? <div className="skeleton h-16 w-full"></div> : (
            <div>
              ...
              <div className="w-full bg-slate-100 dark:bg-dark-700 rounded-full h-3 mb-1 overflow-hidden shadow-inner">
                <div 
        ...
        <div className="premium-card col-span-1 md:col-span-2 lg:col-span-1 row-span-2 flex flex-col">
          ...
          {isLoading ? <div className="skeleton h-48 w-full"></div> : (
        ...
        <div className="premium-card relative overflow-hidden group">
          <div className="flex items-center gap-4 mb-4">
            ...
            <h3 className="font-semibold text-slate-700 dark:text-slate-200">Proyección Abril</h3>
          </div>
          {isLoading ? <div className="skeleton h-10 w-32"></div> : (
            <div>
              <div className="text-4xl font-bold text-indigo-600 dark:text-indigo-400">${proyeccionMensual.toLocaleString()}</div>
        ...
        <div className="premium-card flex flex-col border-red-100 dark:border-red-900/30">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center gap-4">
              <div className="p-3 bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400 rounded-xl">
                <AlertTriangle className="w-6 h-6" />
              </div>
              <h3 className="font-semibold text-slate-700 dark:text-slate-200">Morosidad Crítica (&gt;60 días)</h3>
            </div>
            {!isLoading && (
              <span className="px-3 py-1 bg-red-100 text-red-700 dark:bg-red-900/40 dark:text-red-400 font-bold rounded-full text-sm">
                {morosidadCritica.length}
              </span>
            )}
          </div>
          <div className="flex-1 overflow-y-auto max-h-[160px] pr-2 custom-scrollbar">
            {isLoading ? (
              ...
              <ul className="space-y-2">
                {morosidadCritica.map((s) => (
                  <li key={s.id_socio} className="flex justify-between items-center p-3 bg-slate-50 dark:bg-dark-900/50 rounded-xl border border-slate-100 dark:border-dark-700/50 hover:bg-slate-100 dark:hover:bg-dark-700 transition-colors">
                    <div>
                      <div className="font-medium text-slate-800 dark:text-slate-200 text-sm">{s.nombre} {s.apellido}</div>
                      <div className="text-xs text-slate-500">Plan: {s.plan || 'Mensual (Base)'}</div>
                    </div>
                    <div className="text-xs font-semibold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-2 py-1 rounded">
              ...
              <div className="h-full flex items-center justify-center text-slate-400 text-sm">
                Ningún socio en estado crítico.
              </div>
            )}
          </div>
        </div>
